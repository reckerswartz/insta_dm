...
module Ai
  class VerifiedStoryInsightBuilderTest < ActiveSupport::TestCase
    ProfileStub = Struct.new(:username)

    test "classifies reshare and blocks generation when external usernames and reshare terms are detected" do
    ...
    test "classifies owned profile content and allows generation with strong verified signals" do
      profile = ProfileStub.new("owner_account")
      payload = {
    ...
    test "blocks low-signal content when ownership cannot be verified" do
      profile = ProfileStub.new("owner_account")
      payload = {
        object_detections: [ { label: "person", confidence: 0.9 } ],
        face_count: 0,
    ...
    test "captures source profile references and marks third-party content" do
      profile = ProfileStub.new("owner_account")
      payload = {
    ...
    test "blocks reshared meme-style content even when profile username is present in source refs" do
      profile = ProfileStub.new("owner_account")
      payload = {
    ...
    test "flags low identity likelihood with external usernames as third-party content" do
      profile = ProfileStub.new("owner_account")
      payload = {
