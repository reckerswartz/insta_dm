class EnqueueProfileRefreshForAllAccountsJob < ApplicationJob
  ...
  ACCOUNT_ENQUEUE_STAGGER_SECONDS = ENV.fetch("PROFILE_REFRESH_ACCOUNT_ENQUEUE_STAGGER_SECONDS", "4").to_i.clamp(0, 120)
  ACCOUNT_ENQUEUE_JITTER_SECONDS = ENV.fetch("PROFILE_REFRESH_ACCOUNT_ENQUEUE_JITTER_SECONDS", "2").to_i.clamp(0, 30)
  ...
  def perform(opts = nil, **kwargs)
    ...
    batch[:accounts].each do |account|
      ...
      enqueue_account_job_with_delay!(
        job_class: SyncNextProfilesForAccountJob,
        slot_index: enqueued,
