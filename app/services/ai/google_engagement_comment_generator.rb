...
module Ai
  class GoogleEngagementCommentGenerator
    DEFAULT_TEXT_MODEL = "gemini-2.0-flash".freeze
    MIN_SUGGESTIONS = 5
    MAX_SUGGESTIONS = 8
    ...
    def initialize(client:, model: nil)
      @client = client
      @model = model.to_s.presence || DEFAULT_TEXT_MODEL
    end
    ...
    def generate!(post_payload:, image_description:, topics:, author_type:)
      ...
      resp = @client.generate_text_json!(
        ...
        usage_category: "text_generation",
        usage_context: { workflow: "google_engagement_comment_generator" }
      )

      parsed = resp[:json].is_a?(Hash) ? resp[:json] : {}
      suggestions = Array(parsed["comment_suggestions"]).map { |v| normalize_comment(v) }.compact.uniq
