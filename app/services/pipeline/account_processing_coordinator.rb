module Pipeline
  class AccountProcessingCoordinator
    ...
    def run!
      ...
      # Phase 10 removed the Ops::LocalAiHealth gate. The coordinator
      # used to short-circuit story / feed / profile-scan enqueues when a
      # local Ollama health probe failed; after the Phase 4 NVIDIA
      # migration the cloud provider is the hot path so there's no local
      # stack to gate against. Per-job failures are still surfaced via
      # EnhancedJobRetryStrategies + Ops::IssueTracker.
