require "rails_helper"

RSpec.describe "UI JavaScript Stability (Admin Routes)", :diagnostic, :slow, :external_app, :diagnostic_ui do
  it "captures no JavaScript errors on admin routes" do
    next unless ensure_ui_audit_server!

    report = run_ui_audit(
      routes: %w[
        /admin/background_jobs
        /admin/background_jobs/failures
        /admin/issues
        /admin/storage_ingestions
      ],
      max_actions: Integer(ENV.fetch("UI_AUDIT_MAX_ACTIONS_ADMIN", "5")),
      include_table_actions: ENV.fetch("UI_AUDIT_INCLUDE_TABLE_ACTIONS", "0") == "1",
      include_nav_actions: ENV.fetch("UI_AUDIT_INCLUDE_NAV_ACTIONS", "0") == "1",
    )

    expect(report.dig(:totals, :errors)).to eq(0), format_ui_audit_issues(report)
  end
end
